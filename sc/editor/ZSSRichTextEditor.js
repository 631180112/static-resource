...
var AndroidDelegate = window.AndroidDelegate;
AndroidDelegate.init = function () {};
AndroidDelegate.log  = function (command) {};
AndroidDelegate.onNewField = function (command) {};
AndroidDelegate.onDomLoaded = function (command) {};
...
AndroidDelegate.onSelectionChanged = function (command) {};
...
ZSSEditor.init = function() {
	// Maleskine Initial
   // this.AndroidDelegate = window.AndroidDelegate;
				...
				if (parent.length === 0) {
					...
					if (node) {
						if (!node.classList || !node.classList.contains('field')) {
							...
							console.log('Change Selection In SelectionChange-Event');
	...
	if (sel.rangeCount) {
		...
		if (needsToWorkAroundNewlineBug) {
			...
			if (closerParentNode.id === 'zss_field_content') {
				...
				if (closerParentNode.nodeName.toLowerCase() === 'input') {
					y += closerParentNode.parentElement.offsetTop;
				}
			}
			height = lineHeight;
		}
...
ZSSEditor.MarkdownImageObject.prototype.updateMarkdown = function () {
	if (this.state === 0) {
		...
		this.markdown = '![图片发自TinyBlogApp](' + this.remote + ')';
	}
...
ZSSEditor.insertImage = function(url, alt, display_url, slug, width, height) {
	...
	if (isMarkdown) {
		...
		if (!alt || !alt.length || alt.trim().length === 0) alt = '图片发自TinyBlogApp'; // For iOS line-height bug.
		var html;
		if (!!display_url) {
...
ZSSEditor.setProgressOnImage = function(imageNodeIdentifier, progress) {
	if (isMarkdown) {
		...
		if (progress >= 100){
			...
			pack.find('.image-caption').html('图片发自TinyBlogApp'); // &nbsp; for iOS line-height bug.
			pack.find('.image-caption-input').val('图片发自TinyBlogApp'); // &nbsp; for iOS line-height bug.
		}
...
ZSSField.prototype.handleInputEventForTitle = function (e) {
	...
	AndroidDelegate.getTitle(title);
};
...
Maleskine.getWordage = function(no_send) {
	...
	if (isMarkdown) {
		...
		if (wordage > 0) {
			...
			content = content.replace(Maleskine.RegEx.SYMBOLS, ' $1 ');
			content = content.match(Maleskine.RegEx.WORDS);
			if (content) {
				wordage = content.length;
			}
